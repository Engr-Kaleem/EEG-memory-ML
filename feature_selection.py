...
for sub in range(1,subs+1,1):
    ...
    cmd.plot()
    plt.savefig(f'confmatSMOTE/subj{sub}clf{clf}_{selected}_channels_FS')
plot_metrics_FS(All_meterics,clf,selected)
np.savetxt(f'Accuracey/clssifier{clf}_{selected}_channel_FS.csv', All_meterics,delimiter = ",")