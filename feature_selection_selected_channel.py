...
from Classifier import  FeatureSelect
...
import pandas as pd
import numpy as np
from metrics_plot import plot_metrics_FS
...
for sub in range(1,subs+1,1):
    ...
    fselect=FeatureSelect(n_features_select=4, n_csp_pairs=2)
    features_mat_train = fbcsp.transform(filtered_data_train, class_idx=0)
    features_mat_test = fbcsp.transform(filtered_data_test, class_idx=0)
    if feature_selection:
        x_features_selected_train = fselect.fit(features_mat_train,y_train)
        x_features_selected_test = fselect.transform(features_mat_test)
    else:
        x_features_selected_train=features_mat_train
        x_features_selected_test=features_mat_test
    ...
    print(f'feature shape after feature selection {x_features_selected_train.shape}')
    print(f'feature shape after feature selection transform{x_features_selected_test.shape}')
    #Use SMOTE to oversample the minority class
    oversample = SMOTE()
    over_X, over_y = oversample.fit_resample(x_features_selected_train,y_train)
    ...
    y_pred = SMOTE_SRF.predict(x_features_selected_test)
    ...
    cmd.plot()
    plt.savefig(f'confmatSMOTE/subj{sub}clf{clf}_{selected}_featrue_selected')
   
plot_metrics_FS(All_meterics,clf,selected)
np.savetxt(f'Accuracey/clssifier{clf}_{selected}_channel_selected_feature.csv', All_meterics,delimiter = ",")
